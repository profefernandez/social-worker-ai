 ...
 * @returns {Promise<string|Object>} — text reply, or { content, toolCalls } when returnFullResponse is true
 */
async function sendMessage(input, conversationHistory = [], config = {}) {
  const apiKey = config.apiKey || process.env.MISTRAL_API_KEY;
  if (!apiKey) {
    throw new Error('Mistral API key not configured — set apiKey in config or MISTRAL_API_KEY env var');
  }
...
module.exports = { sendMessage, formatRequest, parseResponse, parseFullResponse, MISTRAL_URL };
