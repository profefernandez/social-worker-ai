  ...
  return (
    <div className="frost-panel rounded-xl flex flex-col h-full overflow-hidden">
      <div className="px-4 py-3 border-b border-ember-text/5 flex items-center gap-2">
        ...
        <span className="ml-auto text-[13px] font-mono text-ember-muted">{entries.length}</span>
      </div>
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        {entries.length === 0 && (
          <p className="text-ember-subtle text-xs text-center py-4 font-mono">Waiting for agent...</p>
        )}
        {entries.map((entry, i) => (
...
function AgentEntry({ entry }) {
  ...
  return (
    <div ref={ref} className="frost-panel rounded-lg px-3 py-2">
      <span className="text-[13px] font-mono text-ember-muted block mb-1">
